...
int main_2()
{
    ...
    auto th = std::thread([&queue](){
        ...
        value_t ret; queue.wait_pull(ret);
        while(ret.has_value())
        {
    ...
    std::cout << "pushing...\n";
    queue.wait_push( value_t( sub_value_t{ req, [](drogon::ReqResult, const drogon::HttpResponsePtr& res) { std::cout << res->getBody() << std::endl; } } ) );
...
struct _MyStruct : public SerialHelper
{
    // explicit _MyStruct(const double s, const std::string n) : score{s}, name{n} {}
    // using SerialHelper::SerialHelper;

    ser<&_MyStruct::_, double> score;
    ser<&_MyStruct::score, std::string> name;
    int agata;

};
using MyStruct = ClassWrapper<&_MyStruct::name>;
