...
namespace core
{
	namespace reports
	{
		...
		void summary::process(publications_storage_t input, const objects::match_type mt) { process_impl(input, mt); }
		...
		void summary::process_impl(publications_storage_t input, const objects::match_type mt) 
		{
			...
			for(size_t i = 0; i < browser->size(); ++i)
			{
				...
				if(x.matched()().data().find(search) != x.matched()().data().end()) continue;	// it's allready matched, no sense for processing
				const auto& ref = (*x.reference()().data())();

				for(const objects::shared_publication_t& y : input)
				{
					if( ref.compare(*y() ) == 0 ) // TODO: compare function is not enough, but fair enough now;
					{
						// objects::publication_with_source_t to_add;
						m_report.access([&](report_t& obj)
						{
							(*(*obj)[i]())().matched()().data().emplace( mt, y );
						});
					}
				}
			}
