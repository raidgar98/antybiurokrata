	...
	namespace network
	{
		...
		scopus_adapter::result_t scopus_adapter::get_person(const str &orcid)
		{
			...
			const auto safe_get = [&null_value, &cengine](const str& field, const jvalue& json) -> u16str
			{
				...
				else return cengine.from_bytes( element.asCString() );
			};
			...
			do
			{
				...
				catch (const std::exception &e) { log.error() << "cought `std::exception` while gathering json. what(): " << logger::endl << e.what() << logger::endl; throw; }
				catch (...) { log.error() << "cought unknown exception while gathering json" << logger::endl; throw; }
